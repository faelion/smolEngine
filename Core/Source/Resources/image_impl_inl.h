...
namespace smol {
	...
	template<>
	inline void Resources::LoadMeta<Texture2D>(const char* file, ImageSettings* settings)
	{
	...
	template<>
	inline void Resources::CreateMeta<Texture2D>(const char* file, ImageSettings* settings)
	{
		if (!settings)
			return;
		std::filesystem::path filePath = file;
