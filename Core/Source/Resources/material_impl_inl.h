...
namespace smol {
	...
	inline void Resources::Import<Material>(const char* file)
	{
		...
		export_file.replace_extension(".smolmaterial");
		...
		SMOL_CORE_INFO("Material at {} imported succesfully!", import_file.string().c_str());
	}
	template<>
	inline bool Resources::CheckImport<Material>(const char* file)
	{
		return _check_import_impl(file, ".smolmaterial");
	}
	template<>
