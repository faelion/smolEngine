...
namespace smol {
	...
	void FrameBuffer::Reset(bool depth)
	{
		...
		if (depth) {
			...
			glTexStorage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, m_Width, m_Height);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		...
		{
			SMOL_CORE_ASSERT_MSG("Framebuffer not completed")
		}
		else
			SMOL_CORE_INFO("Framebuffer completed");
	...
	void FrameBuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			SMOL_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
		m_Width = width;
		m_Height = height;
